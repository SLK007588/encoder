...
from SmartEncoder.Plugins.wrappers import access
...
if __name__ == "__main__":
	...
	@TGBot.on_message(filters.command("test"))
	@access
	async def test(bot: Client, msg: Message):

		await msg.reply("You do have access mate")
	...
	async def strrr(bot: Client, msg: Message):
		try:
			...
			if msg.command[-1] != "start":
				...
				isCor = myDb.accessed(msg.from_user.id, msg.command[-1])

				if isCor:
					await ms.edit("Access Granted")
				else:
					await ms.edit("Access Denied, Check your token again")

		except MessageEmpty:
			await msg.reply("Nothing here, check token again")

		except Exception as e:
			logger.error(f"TOKEN ACCESS : {e}")

	@TGBot.on_message(filters.incoming & filters.private & filters.command(['token']))
	@access
